...
from sub_progress.secret import *
from sub_progress.glossary import sub_gl_en_zh, sub_tichun_zh
...
def tran_google(src_text):
    ...
    if len(src_text) > 12:
        ...
        for i in range(0, src_len):
            print("正在翻译：" + str(i*12) + " | " + str(len(src_text)))
            src_cache2 = '\n'.join('%s' % id for id in src_cache[i])
...
def paste_it(chunk, driver):
    ...
    # lmt__target_toolbar__copy_container button
    button_css = '.lmt__target_toolbar button'

    # Getting the button object
    button = driver.find_elements(By.CSS_SELECTOR, button_css)[-1]
    ...
    #     'document.querySelector(".lmt__side_container--target textarea").select()')
...
def get_driver(src_lang='en'):
    ...
    deepl_url = 'https://www.deepl.com/translator#' + src_lang.upper() + '/ZH/'
    driver.get(deepl_url)
    return driver


def generate_list(src_list):
    chars = 0
    tar_list = []
    cache_list = []
...
def tran_deepl_auto(src_list):
    driver = get_driver()
    list_cache = generate_list(src_list)
    res = []
