    ...
    if not os.path.exists(f"{path}.mp4"):
        ...
        for vtp in video_type:
            if os.path.exists(path + vtp):
                ffmpeg_param1 = (
                    "ffmpeg -i " + '"' + path + vtp + '" ' + '-vcodec copy -acodec copy "' + path + ".mp4" + '"'
                )
                os.system(ffmpeg_param1)

    # 转换图片
    if not os.path.exists(f"{path}.jpg") and not os.path.exists(f"{path}.png"):
        ffmpeg_param2 = "ffmpeg -i " + '"' + path + ".webp" + '" ' + '-vcodec copy -acodec copy "' + path + ".png" + '"'
...
def ytb_dl_download(url: str, out_path: str):
    ...
    if not os.path.exists(f"{out_path}.mkv") and not os.path.exists(f"{out_path}.mp4"):
        ydl_opts = {
            "outtmpl": out_path,
            "writethumbnail": True,
            "verbose": True,
...
def yt_dlp_download(
    url: str,
    output_path: str,
    down_video: bool = False,
    down_sub: bool = False,
    down_thumbnail: bool = False,
    prefix: str = "",
):
    ...
    with YoutubeDL(ydl_opts) as ydl:
        if video_info := ydl.extract_info(url, download=False):
            ...
            if (
                not os.path.exists(f"{current_path}.mkv")
                and not os.path.exists(f"{current_path}.mp4")
                and not os.path.exists(f"{current_path}.webm")
            ):
...
def writeSub(captions):
    ...
    for i in range(len(captions)):
        ...
        sub = pysrt.SubRipItem(
            index=i + 1,
            start=pysrt.SubRipTime(milliseconds=caption["start"] * 1000),
            end=pysrt.SubRipTime(milliseconds=(caption["start"] + caption["duration"]) * 1000),
            text=caption["text"],
        )
        # 将字幕条目添加到字幕对象中
        subs.append(sub)
...
if __name__ == "__main__":
    ...
    yt_dlp_download(url=url, output_path="output", down_sub=True, down_video=True, down_thumbnail=True, prefix="1_")
