...
def todo_ui():
    ...
    ui.linear_progress(sum(item.done for item in todos.items) /
                       len(todos.items), show_value=False)
    ...
    for item in todos.items:
        with ui.row().classes('items-center'):
            ui.checkbox(value=item.done, on_change=todo_ui.refresh).bind_value(
                item, 'done')
            ui.input(value=item.name).classes(
                'flex-grow').bind_value(item, 'name')
...
with ui.card().classes('w-80 items-stretch'):
    ...
    add_input.on('keydown.enter', lambda: (
        todos.add(add_input.value), add_input.set_value('')))
