...
# fork: https://github.com/pengzhile/pandora/blob/master/src/pandora/openai/api.py
...
class API:
    ...
    def _request_sse(self, url, headers, data):
        ...
        result = ""
        end_turn = True
        for item in self.__generate_wrap(queue, t, e):
        ...
        if result:
            end_turn = result['message']['end_turn']
            conversation_id = result["conversation_id"]
            text = result['message']['content']['parts'][0]
            last_message_id = result['message']['id']
            
            return end_turn , text , conversation_id, last_message_id
        else:
            return None , "未获取到信息", None ,None
